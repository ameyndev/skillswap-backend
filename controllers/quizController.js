...
const createQuiz = async (req, res) => {
  try {
    ...
    if (!questions || questions.length < 1 || questions.length > 10) {
      return res.status(400).json({ error: 'Quiz must have between 1 and 10 questions' });
    }
    ...
    for (let i = 0; i < questions.length; i++) {
      ...
      const correctAnswers = question.options.filter(option => option.isCorrect).length;
      if (correctAnswers !== 1) {
        return res.status(400).json({ error: `Question ${i + 1} must have exactly one correct answer` });
      }
    }
...
const sendQuiz = async (req, res) => {
  try {
    ...
    if (quiz.createdBy.toString() !== userId) {
      return res.status(403).json({ error: 'You can only send quizzes you created' });
    }
    ...
    if (skillRequestId) {
      ...
      if (skillRequest.fromUser.toString() !== userId || skillRequest.toUser.toString() !== toUserId) {
        return res.status(403).json({ error: 'Invalid skill request association' });
      }
    }
...
const submitQuiz = async (req, res) => {
  try {
    ...
    if (quiz.sentTo.toString() !== userId) {
      return res.status(403).json({ error: 'You can only submit quizzes sent to you' });
    }
    ...
    await updateUserSkillPoints(userId, quiz.skillName, score, quiz.questions.length);
...
// Helper function to update user skill points
const updateUserSkillPoints = async (userId, skillName, correctAnswers, totalQuestions) => {
  try {
