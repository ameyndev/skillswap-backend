...
io.on('connection', (socket) => {
  ...
  socket.on('join-call', (roomId) => {
    ...
    console.log('Room members now:', io.sockets.adapter.rooms.get(roomId)?.size || 0);
  ...
  socket.on('test-event', (data) => {
    ...
    console.log('Room members:', io.sockets.adapter.rooms.get(data.roomId)?.size || 0);
  ...
  socket.on('call-invitation', (data) => {
    ...
    console.log(`User ${socket.id} sent call invitation to room ${data.roomId}`);
